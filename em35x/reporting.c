...
int8u disableReportMinTime = FALSE;
...
void emberAfPluginReportingTickEventHandler(void)
{
  ...
  for (i = 0; i < EMBER_AF_PLUGIN_REPORTING_TABLE_SIZE; i++) {
    ...
    if ((disableReportMinTime == TRUE)
        && (ZCL_ON_OFF_CLUSTER_ID != entry.clusterId)
        && (ZCL_ON_OFF_ATTRIBUTE_ID != entry.attributeId))
    { continue;}
    ...
    case EMBER_ZCL_REPORTING_DIRECTION_REPORTED:
      {
        ...
        if (metadata != NULL && metadata->attributeType != dataType) {
          ...
          newEntry.mask = mask;
          if (EMBER_AF_MANUFACTURER_CODE == cmd->mfgCode)
          { newEntry.manufacturerCode = EMBER_AF_MANUFACTURER_CODE;}
          else
