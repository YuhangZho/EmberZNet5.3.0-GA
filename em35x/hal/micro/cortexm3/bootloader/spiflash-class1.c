 ...
 * communicate with the external flash. This must be enabled in the case of
 * app bootloading over USB, since USB uses SC2. Also ensure that the board
 * jumpers are in the right place and the debugger is not connected to any SC1
 * pins.
 */
...
typedef enum {
  ...
  #define EEPROM_CS_ACTIVE()   do { GPIO_PBCLR = BIT(4); } while (0)
  // CS pin, deactivate chip; PA3
  #define EEPROM_CS_INACTIVE() do { GPIO_PBSET = BIT(4); } while (0)
#else //SPI_FLASH_SC1
  // CS pin, activate chip; PA3
...
#else
  #define EEPROM_POWER_PIN       PORTA_PIN(7)
  #define SET_EEPROM_POWER_PIN() GPIO_PASET = PA7
  #define CLR_EEPROM_POWER_PIN() GPIO_PACLR = PA7
#endif
...
int8u halEepromInit(void)
{
  DeviceType deviceType;
    ...
    //-----SC1 SPI Master GPIO configuration
    GPIO_PBCFGL =   (GPIOCFG_OUT_ALT  << PB1_CFG_BIT)|  // PB1 MOSI
                    (GPIOCFG_IN       << PB2_CFG_BIT)|  // PB2 MISO
                    (GPIOCFG_OUT_ALT  << PB3_CFG_BIT);  // PB3 SCLK
    GPIO_PBCFGH =   (GPIOCFG_OUT      << PB4_CFG_BIT);  // PB4 nSSEL

    SC1_RATELIN = SPI_LIN;
    SC1_RATEEXP = SPI_EXP;
    SC1_SPICFG  =  0;
    SC1_SPICFG =  (1 << SC_SPIMST_BIT)|  // 4; master control bit
