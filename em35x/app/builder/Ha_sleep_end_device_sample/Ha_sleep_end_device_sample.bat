...
echo " "
@echo on
cmd /c ""%PROJECT_DIR%\..\..\..\tool\image-builder\image-builder-windows.exe" --create %TARGET_BPATH%.ota  --version 1 --manuf-id 0x1160 --image-type 0 --tag-id 0x0000 --tag-file %TARGET_BPATH%.ebl --string "EBL Ha_sleep_end_device_sample" > %TARGET_BPATH%-image-builder-output.txt"
@echo off
