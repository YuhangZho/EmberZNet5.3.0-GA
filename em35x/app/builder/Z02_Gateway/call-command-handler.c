...
EmberAfStatus emberAfClusterSpecificCommandParse(EmberAfClusterCommand *cmd)
{
  if (cmd->direction == ZCL_DIRECTION_SERVER_TO_CLIENT
	...
	case ZCL_GROUPS_CLUSTER_ID:
	  return emberAfGroupClusterClientCommandParse(cmd);
    }
    ...
    case ZCL_MOBILE_CONTROL_CLUSTER_ID:
      return emberAfSengledMobileScenseClusterServerCommandParse(cmd);
    }
  }
...
EmberAfStatus emberAfGroupClusterClientCommandParse (EmberAfClusterCommand *cmd)
{
	boolean wasHandled = FALSE;
	if (!cmd->mfgSpecific) {
		switch (cmd->commandId) {
		...
		{
			int16u groupId = 0xffff;
			int16u payloadOffset = cmd->payloadStartIndex;
			int8u status = emberAfGetInt16u(cmd->buffer, payloadOffset, cmd->bufLen);
			payloadOffset += 1;
...
// Cluster: Mobile Scense, server
EmberAfStatus emberAfSengledMobileScenseClusterServerCommandParse(EmberAfClusterCommand *cmd)
{
  boolean wasHandled = FALSE;
  if (cmd->mfgSpecific) {
    if (cmd->mfgCode == 0x1160 && cmd->commandId == ZCL_MOBILE_CONTROL_COMMAND_ID) {
      int16u payloadOffset = cmd->payloadStartIndex;
      int16u controlType;  // Ver.: always
      int16u controlData;  // Ver.: always
      ...
      controlType = emberAfGetInt16u(cmd->buffer, payloadOffset, cmd->bufLen);
      payloadOffset += 2;
      controlData = emberAfGetInt16u(cmd->buffer, payloadOffset, cmd->bufLen);
      payloadOffset += 2;
      transitionTime = emberAfGetInt16u(cmd->buffer, payloadOffset, cmd->bufLen);
      wasHandled = emberAfSengledMobileScenseClusterMobileControlCallback(controlType,
                                                                          controlData,
                                                                          transitionTime,
                                                                          cmd->seqNum);
    }
