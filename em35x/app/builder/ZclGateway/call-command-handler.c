...
EmberAfStatus emberAfClusterSpecificCommandParse(EmberAfClusterCommand *cmd)
{
  if (cmd->direction == ZCL_DIRECTION_SERVER_TO_CLIENT
    ...
    case ZCL_MOBILE_CONTROL_CLUSTER_ID:
      return emberAfMobileControlClusterServerCommandParse(cmd);
    }
  }
...
// Cluster: mobile control, server
EmberAfStatus emberAfMobileControlClusterServerCommandParse(EmberAfClusterCommand *cmd)
{
  boolean wasHandled = FALSE;
  if (cmd->mfgSpecific) {
    if (cmd->mfgCode == 0x1160 && cmd->commandId == ZCL_MOBILE_CONTROL_COMMAND_ID) {
      int16u payloadOffset = cmd->payloadStartIndex;
      int16u control_type;  // Ver.: always
      int16u control_data;  // Ver.: always
      int16u transition_time;  // Ver.: always
      // Command is fixed length: 6
      if (cmd->bufLen < payloadOffset + 6) return EMBER_ZCL_STATUS_MALFORMED_COMMAND;
      control_type = emberAfGetInt16u(cmd->buffer, payloadOffset, cmd->bufLen);
      payloadOffset += 2;
      control_data = emberAfGetInt16u(cmd->buffer, payloadOffset, cmd->bufLen);
      payloadOffset += 2;
      transition_time = emberAfGetInt16u(cmd->buffer, payloadOffset, cmd->bufLen);
      wasHandled = emberAfSengledMobileScenseClusterMobileControlCallback(control_type,
                                                                     control_data,
                                                                     transition_time);
    }
  }
