...
/** @brief <b>plugin ota-server notify [destination:2] [endpoint:1] [payloadType:1] [jitter:1] [manuf-id:2] [imageTypeId:2] [version:2] </b>
 ...
 *     - version - INT16U - Firmware version of the image being advertised (should match the version from the OTA file's header)
 */
...
/** @brief <b>plugin ota-server policy image-req-min-period [policyValue:1] </b>
 *   - <i></i>
 *     - policyValue - INT8U - 0: Upgrade if server has newer (default), 1: Downgrade if server has older, 2:       Reinstall if server has same, 3: No next version (no next image is available for download)
 */
