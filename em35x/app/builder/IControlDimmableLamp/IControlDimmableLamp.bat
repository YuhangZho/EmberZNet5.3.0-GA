...
echo " "
@echo on
cmd /c ""%PROJECT_DIR%\..\..\..\tool\image-builder\image-builder-windows.exe" --create %TARGET_BPATH%.ota  --version 5 --manuf-id 0x1901 --image-type 0x0101 --tag-id 0x0000 --tag-file %TARGET_BPATH%.ebl --string "EBL IControlDimmableLamp" > %TARGET_BPATH%-image-builder-output.txt"
@echo off
