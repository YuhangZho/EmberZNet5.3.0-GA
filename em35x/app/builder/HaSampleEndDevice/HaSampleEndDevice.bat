...
echo " "
@echo on
cmd /c ""%PROJECT_DIR%\..\..\..\tool\image-builder\image-builder-windows.exe" --create %TARGET_BPATH%.ota  --version 256 --manuf-id 0x1160 --image-type 0 --tag-id 0x0000 --tag-file %TARGET_BPATH%.ebl --string "EBL HaSampleEndDevice" > %TARGET_BPATH%-image-builder-output.txt"
@echo off
