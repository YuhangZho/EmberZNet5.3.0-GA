    ...
    case ZCL_SCAN_REQUEST_COMMAND_ID:
      {
        ...
        wasHandled = emberAfZllCommissioningClusterScanRequestCallback(transaction,
                                                                       zigbeeInformation,
                                                                       zllInformation);
        break;
      }
    case ZCL_DEVICE_INFORMATION_REQUEST_COMMAND_ID:
      {
        ...
        wasHandled = emberAfZllCommissioningClusterDeviceInformationRequestCallback(transaction,
                                                                                    startIndex);
        break;
      }
    case ZCL_IDENTIFY_REQUEST_COMMAND_ID:
      {
        int16u payloadOffset = cmd->payloadStartIndex;
        int32u transaction;  // Ver.: always
        int16u identifyDuration;  // Ver.: always
        // Command is fixed length: 6
        if (cmd->bufLen < payloadOffset + 6) return EMBER_ZCL_STATUS_MALFORMED_COMMAND;
        transaction = emberAfGetInt32u(cmd->buffer, payloadOffset, cmd->bufLen);
        payloadOffset += 4;
        identifyDuration = emberAfGetInt16u(cmd->buffer, payloadOffset, cmd->bufLen);
        wasHandled = emberAfZllCommissioningClusterIdentifyRequestCallback(transaction,
                                                                           identifyDuration);
        break;
      }
    ...
    case ZCL_NETWORK_START_REQUEST_COMMAND_ID:
      {
        ...
        int8u keyIndex;  // Ver.: always
        int8u* encryptedNetworkKey;  // Ver.: always
        int8u logicalChannel;  // Ver.: always
        int16u panId;  // Ver.: always
        int16u networkAddress;  // Ver.: always
        int16u groupIdentifiersBegin;  // Ver.: always
        int16u groupIdentifiersEnd;  // Ver.: always
        int16u freeNetworkAddressRangeBegin;  // Ver.: always
        int16u freeNetworkAddressRangeEnd;  // Ver.: always
        int16u freeGroupIdentifierRangeBegin;  // Ver.: always
        int16u freeGroupIdentifierRangeEnd;  // Ver.: always
        int8u* initiatorIeeeAddress;  // Ver.: always
        int16u initiatorNetworkAddress;  // Ver.: always
        // Command is fixed length: 56
        if (cmd->bufLen < payloadOffset + 56) return EMBER_ZCL_STATUS_MALFORMED_COMMAND;
        transaction = emberAfGetInt32u(cmd->buffer, payloadOffset, cmd->bufLen);
        payloadOffset += 4;
        ...
        wasHandled = emberAfZllCommissioningClusterNetworkStartRequestCallback(transaction,
                                                                               extendedPanId,
                                                                               keyIndex,
                                                                               encryptedNetworkKey,
                                                                               logicalChannel,
    ...
    case ZCL_NETWORK_JOIN_ROUTER_REQUEST_COMMAND_ID:
      {
        ...
        int16u freeGroupIdentifierRangeBegin;  // Ver.: always
        int16u freeGroupIdentifierRangeEnd;  // Ver.: always
        // Command is fixed length: 47
        if (cmd->bufLen < payloadOffset + 47) return EMBER_ZCL_STATUS_MALFORMED_COMMAND;
        transaction = emberAfGetInt32u(cmd->buffer, payloadOffset, cmd->bufLen);
        payloadOffset += 4;
        extendedPanId = cmd->buffer + payloadOffset;
        payloadOffset += 8;
    ...
    case ZCL_NETWORK_JOIN_END_DEVICE_REQUEST_COMMAND_ID:
      {
        ...
        payloadOffset += 2;
        freeGroupIdentifierRangeEnd = emberAfGetInt16u(cmd->buffer, payloadOffset, cmd->bufLen);
        wasHandled = emberAfZllCommissioningClusterNetworkJoinEndDeviceRequestCallback(transaction,
                                                                                       extendedPanId,
                                                                                       keyIndex,
                                                                                       encryptedNetworkKey,
                                                                                       networkUpdateId,
    ...
    case ZCL_NETWORK_UPDATE_REQUEST_COMMAND_ID:
      {
        ...
        payloadOffset += 2;
        networkAddress = emberAfGetInt16u(cmd->buffer, payloadOffset, cmd->bufLen);
        wasHandled = emberAfZllCommissioningClusterNetworkUpdateRequestCallback(transaction,
                                                                                extendedPanId,
                                                                                networkUpdateId,
                                                                                logicalChannel,
                                                                                panId,
