...
void zdoActiveEpCommand(void)
{
  ...
  emberAfAppPrint("Active EP request status: 0x%X, target: 0x%2X",
                  status,target);
}
...
void zdoLeaveRequestCommand(void)
{
  ...
  EmberStatus status = emberLeaveRequest(target,
                                         ...
                                         EMBER_APS_OPTION_RETRY);
#else
	EmberEUI64 mac_gateway={0xa5,0x39,0x81,0x01,0x00,0x6f,0x0D,0x00}; 
	EmberEUI64 mac_bulb_to_leave={0xE3,0x29,0x55,0x0C,0x00,0x6F,0x0D,0x00}; 

	EmberStatus status = emberSendRemoveDevice(0x0000,			//mac node id;
									mac_gateway,
									mac_bulb_to_leave);
#endif
