...
void zclGlobalSetup(int8u commandId)
{
  ...
  // Bug: ZclGateway 20170804: read ota information ,that's a server to client request
  if(clusterId == 0x0019)
  {
	  zclGlobalDirection = ZCL_FRAME_CONTROL_SERVER_TO_CLIENT;
  }
  ...
  {
    // make a test, continue send zcl global read with fixed short address
    
    // read the short addr;
    int16u dest_short_addr = (int16u)emberUnsignedCommandArgument(2);
	// prepare work is ready, send it!
...
void zclScenesAddCommand(void)
{
  ...
  zclBufferAddLengthAndStringFromArgument(3);
  emberAfGuaranteedPrintln("SCENE START?");

#ifdef ZCL_USING_ON_OFF_CLUSTER_CLIENT
  emberAfGuaranteedPrint("ZCL_USING_ON_OFF_CLUSTER_CLIENT");
...
#ifdef ZCL_USING_LEVEL_CONTROL_CLUSTER_CLIENT
  emberAfGuaranteedPrint("ZCL_USING_LEVEL_CONTROL_CLUSTER_CLIENT");
...
void emAfCliSendCommand(void)
{
  ...
  if (commandName[4] == '_') {
    label = 'M';
	emberAfCorePrintln("Multicast: src: %x, dest:%x, groupId:%2x",
						srcEndpoint,dstEndpoint,destination);
