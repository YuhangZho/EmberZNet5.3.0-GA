...
#include "sengled-hardware-Highbay.h"
#include "app/framework/plugin/sengled-ha-common-Highbay/sengled-ha-common.h"
...
int8u doubleClickCount = 0;
boolean doubleClickStart = FALSE;
boolean doubleClickEnd = TRUE;
static int16u powerDetectTimeStart = 0;
static int16u powerDetectTimeEnd = 0;
...
#ifdef ROBIN_DEBUG
		emberAfGuaranteedPrintln("__ISRIN__,state:%x,%x",button,state); 
#endif
/*
  if(button == DOUBLE_CLICK)
  {
    if (state == DOUBLE_CLICK_ON)
    {
      ...
      if(doubleClickStart == TRUE)
      {
		...
		powerDetectTimeEnd = halCommonGetInt16uMillisecondTick();
		if((powerDetectTimeEnd - powerDetectTimeStart)>=30)
		{
			doubleClickEnd = TRUE;
			doubleClickCount++;
	...
	else if(state == DOUBLE_CLICK_OFF)
	{
	  ...
	  powerDetectTimeStart = halCommonGetInt16uMillisecondTick();
