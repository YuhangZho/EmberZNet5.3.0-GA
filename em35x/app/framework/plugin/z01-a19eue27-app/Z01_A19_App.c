...
#define SET_LAMP_OFF()  do { GPIO_PBSET = BIT(3); } while (0)
#define SET_LAMP_ON()   do { GPIO_PBCLR = BIT(3); } while (0)

// level
#define DIMMING_MAX_LEVEL  255
#define DIMMING_MIN_LEVEL  0
#define DIMMING_BINLK_MIN_LEVEL  1
...
void SetLedCct(int16u cct)
{
  ...
  //emberAfGuaranteedPrint("Cct = %d, %d \r\n", cctPwm, cct);
}
...
void emberAfPluginIdentifyFeedbackProvideFeedbackEventHandler(void)
{
  ...
  { 
    flag = TRUE;
    SetLedDimming(DIMMING_BINLK_MIN_LEVEL);
  }
...
{  
  if (indicateCount >= indicateCountMax[indicateType])
  {
    ...
    { 
      ...
      emberEventControlSetDelayMS(indicateEventControl, 10);
    }
...
{ 
  ...
  {   
    ...
    if (state == BUTTON_RELEASED)
    {
      //emberAfGuaranteedPrint("BUTTON_RELEASED\r\n");
      if (onOffCount == 0)
      { return;}
      ...
      if (2 != onOffCount)
      { emberEventControlSetDelayMS(buttonEventControl, 1200);}
      emberEventControlSetDelayMS(SwitchOvertimeEventControl, 2000);
    }
    else
    {
      oldTime = halCommonGetInt16uMillisecondTick();
      //emberAfGuaranteedPrint("BUTTON_PRESSED\r\n");
      onOffCount++;
      emberEventControlSetInactive(buttonEventControl);
      if ((2 == onOffCount) && (TRUE == dimmingFlag))
      { emberEventControlSetActive(buttonEventControl);}           
      emberEventControlSetDelayMS(SwitchOvertimeEventControl, 1000);
    }
  }
}
...
int32s GetEfficiency(int8u level)
{
  ...
  if (saveCctLeve == colorTempPhysicalMin)
  ...
  {     
    ...
    #endif  
    {
      if (adcChannelIndex == 0)
      {
        ...
        {
          //emberAfGuaranteedPrint("%d, %d, ",currentDifference, currentIn);
...
// 99: firmware version
//*******************************************************************************
int8u firmwareVersion[] = "Z01-A60EAE27-v1.0.7"; //"Z01-A60EAB22-v1.0.3";
