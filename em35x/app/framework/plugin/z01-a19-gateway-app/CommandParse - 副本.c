...
#define FIRMWARE_VERSION (0x00000008ul)
...
{ 
  ...
  *((int16u*)&sBuf[sLen]) = nodeId; //nwkAddr
  ...
  emberActiveEndpointsRequest(nodeId, EMBER_APS_OPTION_RETRY);
...
static void processOtaUploaderReq(int8u *rBuf, int8u len)
{
  ...
  flag = emberAfOtaStorageDriverWriteCallback(&rBuf[9], *(int32u *)&rBuf[1], *(int32u *)&rBuf[5]);
                                   ...
                                   EmberStatus status)
{
  ...
  else if (HA_PROFILE_ID == apsFrame->profileId)
  {
    ...
    if (message[messageLen]&ZCL_MANUFACTURER_SPECIFIC_MASK)
    { messageLen = 2;}
    else
    { messageLen = 0;}
    
    if (0 == (message[messageLen]&ZCL_CLUSTER_SPECIFIC_COMMAND))
    { 
