...
static State states[EMBER_SUPPORTED_NETWORKS] = {0, 0};
...
void emberAfStopMoveCallback(void)
{
  ...
  states[networkIndex].totalMoveAttempts = 0;
...
void emberAfPluginEndDeviceSupportMoveNetworkEventHandler(void)
{
  ...
  State *state = &states[networkIndex];
  EmberStatus status;
  boolean secure = ((state->moveAttempts & 0x01) == 0);
  int32u channels = (state->moveAttempts == 0
                     ? 0 // current channel
                     : EMBER_ALL_802_15_4_CHANNELS_MASK);
  //*****************************************************
  //sengled wms 2015.11.05
  if (state->totalMoveAttempts > 200)
  {
    //emberAfGuaranteedPrint("totalMoveAttempts = %d\r\n", state->totalMoveAttempts);
    state->moveAttempts = 0;
    state->totalMoveAttempts = 0;
  }
  else if (state->totalMoveAttempts >= 85)
  {    
    state->totalMoveAttempts++;
    emAfScheduleMoveEvent();
    //emberAfGuaranteedPrint("totalMove = %d\r\n", state->totalMoveAttempts);
    return;
  }
  //*****************************************************
  ...
  //emberAfGuaranteedPrint("Move attempt\r\n");
  ...
  if (status == EMBER_SUCCESS) {
    state->moveAttempts++;
    state->totalMoveAttempts++;
    //emberAfGuaranteedPrint("moveAttempts = %d, %d\r\n", state->moveAttempts, state->totalMoveAttempts);
  }
  emAfScheduleMoveEvent();
}
...
void emberAfPluginEndDeviceSupportStackStatusCallback(EmberStatus status)
{
  ...
  if (status == EMBER_NETWORK_DOWN || status == EMBER_MOVE_FAILED) { //wms 2015.10.19
    //if (!emberStackIsPerformingRejoin()) {  //wms 2015.3.12
