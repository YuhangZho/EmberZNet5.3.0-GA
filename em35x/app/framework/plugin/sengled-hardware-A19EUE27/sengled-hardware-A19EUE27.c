...
#include "sengled-hardware-A19EUE27.h"
...
boolean doubleClickStart = FALSE;
...
int16u doubleClickOldTime = 0x0000;
boolean softwareOnTiggerHappen = FALSE;

extern int16s adcData[2];
...
EmberEventControl  elecCurtEventControl;
...
void elecCurtFunction(void)
{
	int16s currentIn;
	
	emberEventControlSetInactive(elecCurtEventControl);
...
#ifdef ROBIN_DEBUG
	emberAfGuaranteedPrintln("elecCurt--currentIn:%2x",currentIn); // Robin add for debug
#endif	
	if(currentIn > 0x0100)
	{
		// it's a dimmer change, not power off;
#ifdef ROBIN_DEBUG
		emberAfGuaranteedPrintln("DIMMER, NOT POWER Off"); // Robin add for debug
#endif
	}
	else
	{
		if(softwareOnTiggerHappen == TRUE)
		{
			// when dimmer is working, software on will cause a down edge, avoid it; 
#ifdef ROBIN_DEBUG
			emberAfGuaranteedPrintln("Software_Trigger_DOUBLE_CLICK_OFF"); // Robin add for debug
#endif
		}
		else
		{
#ifdef ROBIN_DEBUG
			emberAfGuaranteedPrintln("DOUBLE_CLICK: TRUE_OFF"); // Robin add for debug
#endif
			...
			emberEventControlSetDelayMS(onOffStatusWhenPowerOffEventControl,120);				
		}	
	}
}
                                 ...
                                 int8u state)
{
	...
	static int16u newTime;  // 2016.3.23 power on affect another when they all supplied by one power
#endif

	if((button == DOUBLE_CLICK) && (doubleClickEnable == TRUE))
	{
		if (state == DOUBLE_CLICK_ON)
		{
			...
			if(doubleClickStart == TRUE)
			{
        		...
        		if (50 > elapsedTimeInt16u(doubleClickOldTime, newTime))
		        {
		        	//do nothing to filter signal burr
#ifdef ROBIN_DEBUG
					emberAfGuaranteedPrintln("FILTER Signal Burr"); // Robin add for debug
#endif
		        }
				else
				{
...
#endif	  
			emberEventControlSetDelayMS(elecCurtEventControl,50);
		}
		return;
	}
	...
	{ 
		// do not open these print, because IRQ_C casue very quickly,
		// and this will cause a reset frequently; 
#ifdef ROBIN_DEBUG
//		emberAfGuaranteedPrintln("DIMMER_DETECT"); // Robin add for debug
#endif

		if((state == BUTTON_RELEASED) && (doubleClickEnable == FALSE))
    	{
...
//			emberAfGuaranteedPrintln("dWFCtr: %2x",dimmerWaveFormCounter); // Robin add for debug
		...
		else if((state == BUTTON_PRESSED) && (doubleClickEnable == FALSE))
		{
#ifdef ROBIN_DEBUG
