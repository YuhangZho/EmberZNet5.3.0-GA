...
typedef struct _SerialPortStr
{
  struct
  {
    int8u buffer[EMBER_SERIAL1_RX_QUEUE_SIZE];
    int8u ptr;
    int8u len;
    int8u checkSum;
    int32u timer;
  ...
  struct
  {
    int8u buffer[EMBER_SERIAL1_TX_QUEUE_SIZE];
    int8u retryCount;
    int8u checkSum;
  } send;
} SerialPortStr;
...
#include "app/framework/plugin/gateway-app/CommandParse.h"
...
boolean emberProcessCommand(int8u *input, int8u sizeOrPort)
{
  ...
  while (EMBER_SUCCESS == emberSerialReadByte(sizeOrPort, &dat))
  {
    ...
    {
      if (serialPortData.rcv.ptr < EMBER_SERIAL1_RX_QUEUE_SIZE)  // Put to buffer
      {
