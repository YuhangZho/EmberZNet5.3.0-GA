  ...
  for (i = 0; i < EMBER_AF_PLUGIN_REPORTING_TABLE_SIZE; i++) {
        ...
        || (!volatileData[i].reportableChange
            && (elapsedMs
                < (entry.data.reported.maxInterval
                   * MILLISECOND_TICKS_PER_SECOND)))) {
#ifdef ROBIN_DEBUG
		emberAfReportingPrintln("Report PASS BY,cluster: 0x%2x",
								entry.clusterId);
#endif
      continue;
    }
    ...
    if (apsFrame != NULL &&
        !(entry.endpoint == apsFrame->sourceEndpoint
          ...
          && entry.manufacturerCode == manufacturerCode)) {
#ifdef ROBIN_DEBUG
			emberAfReportingPrintln("Step 1,cluster: 0x%2x",
										entry.clusterId);
#endif
...
#else
    if ((ZCL_SIMPLE_METERING_CLUSTER_ID == entry.clusterId)  //sengled wms 2015.7.7 
        && (ZCL_CURRENT_SUMMATION_DELIVERED_ATTRIBUTE_ID == entry.attributeId))
    { readData[4] = 0;readData[5] = 0;}
...
*/
#else
		MEMCOPY(&volatileData[i].lastReportValue, readData, 4);
#endif
    }
  }
...
#ifdef RECEIVE_BROADCAST_NOT_REPORT
extern int8u recieve_cmd_type;
#endif

static void conditionallySendReport(int8u endpoint, EmberAfClusterId clusterId)
{
#ifdef ROBIN_DEBUG
    emberAfGuaranteedPrintln("conditionallySendReport,%2x",clusterId); // ROBIN REPORT
#endif
#ifdef RECEIVE_BROADCAST_NOT_REPORT
	if ((recieve_cmd_type == EMBER_INCOMING_BROADCAST) || (recieve_cmd_type == EMBER_INCOMING_MULTICAST))
	{
		recieve_cmd_type = EMBER_INCOMING_UNICAST;
	}
	else
	{
                                             ...
                                             int8u *data)
{
  ...
  for (i = 0; i < EMBER_AF_PLUGIN_REPORTING_TABLE_SIZE; i++) {
    ...
    if (entry.direction == EMBER_ZCL_REPORTING_DIRECTION_REPORTED
      ...
      #else        
      if ((analogOrDiscrete == EMBER_AF_DATA_TYPE_DISCRETE)  //senled 2014.11.24
          || (analogOrDiscrete == EMBER_AF_DATA_TYPE_ANALOG
              && entry.data.reported.reportableChange <= difference))        
      #endif        
      {
                                             ...
                                             int8u *data)
{
  ...
  for (i = 0; i < EMBER_AF_PLUGIN_REPORTING_TABLE_SIZE; i++) {
    ...
    if (entry.direction == EMBER_ZCL_REPORTING_DIRECTION_REPORTED
      ...
      if((0x0702 == clusterId) && (0x0000 == attributeId))
      {
		  	difference = emberAfGetDifference(data,
												   volatileData[i].lastReportValue,
												   4);
	  }
