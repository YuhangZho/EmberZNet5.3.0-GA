...
void emberAfPluginReportingTickEventHandler(void)
{
  ...
  for (i = 0; i < EMBER_AF_PLUGIN_REPORTING_TABLE_SIZE; i++) {
    ...
    dataSize = (dataSize>4)?4:dataSize;  //sengled wms 2015.7.6
...
static void conditionallySendReport(int8u endpoint, EmberAfClusterId clusterId)
{
  //emberAfGuaranteedPrintln("conditionallySendReport,%2x,%x",clusterId,recieve_cmd_type); // ROBIN REPORT
#ifdef RECEIVE_BROADCAST_NOT_REPORT
...
EmberStatus emberAfClearReportTableCallback(void)
{
  ...
  //emberAfGuaranteedPrintln("AfClearReportTable"); // ROBIN REPORT
...
EmberStatus emAfPluginReportingRemoveEntry(int8u index)
{
  ...
  //emberAfGuaranteedPrintln("ReportingRemoveEntry"); // ROBIN REPORT
