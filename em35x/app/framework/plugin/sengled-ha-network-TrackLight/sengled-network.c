...
#include "app/framework/plugin/sengled-hardware-TrackLight/sengled-hardware-TrackLight.h"
#include "sengled-network.h"
#include "app/framework/plugin/sengled-hardware-TrackLight/sengled-ledControl.h"
#include "app/framework/plugin/sengled-ha-common-TrackLight/sengled-ha-token.h"
		...
		case EMBER_JOINED_NETWORK:  // joined  
		{
			if(rejoinHappened == 1) {
				...
				if(joinNetworkFirstTimeFlag == 0x00) { 
					...
					emberAfGuaranteedPrintln("joinNetworkFirstTimeFlag == 0x00,%d",currentLevel); // Robin add for debug					
					emberAfGuaranteedPrintln("autoResetMark,%d",autoResetMark); // Robin add for debug
