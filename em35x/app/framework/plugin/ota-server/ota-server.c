...
extern boolean fobidden_ota_image_response;
                                          ...
                                          int16u* hardwareVersion)
{
  ...
  status = emberAfOtaServerQueryCallback(currentImageId,
    ...
    // BUG: C-M-171026-01, sengled
  if (fobidden_ota_image_response == TRUE && status == EMBER_ZCL_STATUS_SUCCESS)
  {
    // forbidden_ota_image_response = true, 
    // means gateway permit join is open;
    // Joining zigbee network is happening now!
    // so we should forbid ota process through change  status;
