...
#else
  int16u minBlockRequestPeriodTemp;
  emberAfReadClientAttribute(emberAfPrimaryEndpoint(),
                            ZCL_OTA_BOOTLOAD_CLUSTER_ID,
                            ZCL_MINIMUM_BLOCK_REQUEST_PERIOD_ATTRIBUTE_ID,
                            (int8u *)&minBlockRequestPeriodTemp,
                            sizeof(int16u));

  // Assume we can download as fast as we want until told otherwise by server.
  updateMinBlockRequestPeriodAttribute(minBlockRequestPeriodTemp); // minBlockRequestPeriod.
#endif  
...
int16u compareModelIdAndReturnTheRightImageType(int8u *data)
{
	int8u stringCount= 0x00;
	stringCount = *data;

	if(data[1] == 'E' && data[2] == '1' && data[3] == 'K' && data[4] == '-' 
		&& data[5] == 'C' && data[6] == 'C' && data[7] == 'A')
	{
		return 0x07D4;
	}
	else if(data[1] == 'E' && data[2] == '1' && data[3] == 'K' && data[4] == '-' 
		&& data[5] == 'C' && data[6] == 'E' && data[7] == 'A')
	{
		return 0x07D5;
	}
	else if(data[1] == 'E' && data[2] == '1' && data[3] == 'E' && data[4] == '-' 
		&& data[5] == 'C' && data[6] == 'C' && data[7] == 'A')
	{
		return 0x07D8;
	}
  ...
  switch(cmdId) {
    case ZCL_QUERY_NEXT_IMAGE_REQUEST_COMMAND_ID: {
      ...
      emberAfPutInt8uInResp(fieldControl);
#ifdef CHANGE_IMAGETYPE_FOR_SPEC_OTA  
		EmberAfStatus status;
		// Step 1: Read Local Model ID(String) + ImageTypeID(intu16)
		int8u data[ATTRIBUTE_LARGEST];
		int8u dataType;
  
		status = emberAfReadAttribute(0x01,
                                	0x0000,
                                	0x0005,
                                 	CLUSTER_MASK_SERVER,
                                	data,
		...
		if (status == EMBER_ZCL_STATUS_SUCCESS) {
			// Step 2: Is the ImageTypeID is compatible with the Model ID ?
			// 		-->> Yes: Do the image query request with the correct parameters;
			//		-->> No: Update the right ImageTypeID, and then send the request
			currentDownloadFile.imageTypeId = compareModelIdAndReturnTheRightImageType(data);
			//emberAfGuaranteedPrintln("new imageType: %2x",currentDownloadFile.imageTypeId);			
		} 
		else {
			// may need update to start a timer to read until suceed?
			return;
		}		
/*
		emberAfGuaranteedPrintln("QUERY_NEXT_IMAGE: fieldControl:%x, \n manuId:%2x,\n imageType:%2x, \n fimrwareV:%4x", 
	   							fieldControl,
	   							currentDownloadFile.manufacturerId,
	   							currentDownloadFile.imageTypeId,
	   							currentDownloadFile.firmwareVersion);
*/
#endif
    ...
    case ZCL_IMAGE_BLOCK_REQUEST_COMMAND_ID:
    case ZCL_IMAGE_PAGE_REQUEST_COMMAND_ID: 
	  // if we support MinimumBlockPeriod,we should change fieldControl 
	  // according to the 09-5264-21-zigbee-ota-upgrade-cluster-spec 
	  // bit0 : IEEE address request
	  // bit1 : Minimum Block Request Support
