...
enum {
  ...
  CMD2_REQ_NEW_NODE_ID_BY_MAC = 132
};
...
#define HUB_FIRMWARE_VERSION (0x00001388ul) // V5000
...
static const int8u model[] = "JiaJu-Gateway";
...
void processDebugInfo(int16u debug_info);
...
static void ProcessFindNewNodeId(int8u *rBuf, int8u len)
{
	EmberEUI64 currentEui64;
	MEMCOPY(currentEui64, rBuf, EUI64_SIZE);

	EmberStatus status = emberNetworkAddressRequest(currentEui64,
										  FALSE,		 // report kids?
										  0);			 // child start index;
	...
	*((int16u*)sBuf) = CMD1_ZIGBEE_TYPE; //SUBSYS_ID
	sLen += 2;  
	*((int16u*)&sBuf[sLen]) = CMD2_REQ_NEW_NODE_ID_BY_MAC; //CMD_ID
	sLen += 2;
...
static void ProcessSetPermitJoiningTimeReq(int8u *rBuf, int8u len)
{
	...
	*((int16u*)sBuf) = CMD1_ZIGBEE_TYPE;
	sLen += 2;  
	*((int16u*)&sBuf[sLen]) = CMD2_SET_PERMIT_TIME_TYPE;
	sLen += 2;
	sBuf[sLen++] = (status==EMBER_SUCCESS)?0:1;  //flag
	sBuf[sLen++] = status; //State
...
{ 
  ...
  *((int16u*)sBuf) = CMD1_ZIGBEE_TYPE; 
  sLen += 2;  
  *((int16u*)&sBuf[sLen]) = CMD2_NEW_DEVICE_REPORT_TYPE; 
  sLen += 2;
  sBuf[sLen] = 0x00; //flag
  sLen += 1;
...
int16u GetInformation(int16u cluster, int16u attribute)
{
  ...
  else if (cluster == ZCL_DIAGNOSTICS_CLUSTER_ID)
  {
	...
	else if (cluster == ZCL_PAR38_CONTROL_CLUSTER_ID)
	{
		...
		information = attribute + ATTRIBUTE_PAR38_LUX_THRESHOLD;
	}
                                              ...
                                              int16u bufLen)
{
	...
	*((int16u*)&sBuf[sLen]) = CMD2_GET_ATTRIBUTE_TYPE; sLen += 2; 
...
static void processGetDeviceAttributeReq(int8u *rBuf, int8u len)
{
	...
	if (COORDINATOR_NETWORK_ID == *((int16u*)rBuf))
	{
		...
		*((int16u*)&sBuf[sLen]) = information; sLen += 2; //information
    	...
    	{ 
    		sBuf[sLen++] = emberAfNetworkState();
		}
    	else if (information == COOR_CHANNEL)
    	{ 
    		sBuf[sLen++] = emberGetRadioChannel(); 
		}
                                                   ...
                                                   int16u bufLen)
{
	...
	sBuf = GetSendBuffer();
	*((int16u*)sBuf) = CMD1_ZIGBEE_TYPE; sLen += 2; 
	*((int16u*)&sBuf[sLen]) = CMD2_REPORTING_TYPE; sLen += 2; 
	sBuf[sLen++] = 0x00;    //Flag
	sBuf[sLen++] = 0x00;    //state  
	*((int16u*)&sBuf[sLen]) = emberAfCurrentCommand()->source; sLen += 2;  //desNwkAddr
	...
	else if (clusterId == ZCL_AUTO_RESET_CLUSTER_ID)
	{
		*((int16u*)&sBuf[sLen]) = ATTRIBUTE_LIGHT_AUTO_RESET_MARK; sLen += 2; //attributeId
		sBuf[sLen++] = buffer[bufIndex++]; // one byte information
		for (i=1; i<8; i++)
		{ 
			sBuf[sLen++] = 0;
		}		
	...
	emberGetLastHopLqi(&sBuf[sLen++]);
	emberGetLastHopRssi(&sBuf[sLen++]);
	sBuf[sLen++] = emberAfCurrentCommand()->seqNum;
                                             ...
                                             int16u length)
{
	...
	if(apsFrame->clusterId == PERMIT_JOINING_REQUEST && apsFrame->profileId == 0x0000) 
	{
...
static void processLevelReq(int8u *rBuf, int8u len)
{
	...
	if(len == 0x1D) 
	{
		// 0x1C means it's a new version command which contains judgement for multicast
...
/*********************************************************************
* Support Scene Function for JiaJu Projects
* Robin
* 2018/07/18
* cmd1 : 0x0004
* cmd2：0x0011     
* 举例说明：
* 网关执行创建场景1(编号0x01)命令(对应组id = 0x0002)，
* 变化时间:0x0001, 设备地址：0xDDDD，携带灯具信息：
* 开关(0x0006) = 0x01,  亮度(0x0008) = 0xff, 
* 色温(0x0300) = 0x0055, RGB(0x0300), currentX = 0x1234, currentY = 0xabcd  
* a5 5a 1c | 04 00 | 11 00 | DD DD|  02 00 01 | 01 00 | 06 00 01 01 | 08 00 01 ff | 00 03 06 55 00 34 12 cd ab | fe
*
* Response：
* a5 5a 0d 04 00 11 00 00 00 DD DD  02 00 01 ff ff  D0  
*
*********************************************************************/
void ProcessAddScene(int8u *rBuf, int8u len)
{
	int16u group_id = 0xffff;
	int16u node_id = 0xffff;
	int8u scene_id = 0xff;
	int16u transition_time = 0x0000;
	int8u scene_name[19] = "Sengled_Scene_0000";

	int16u cluster_1 = 0xffff; int8u num1 = 0xff; int8u on_off = 1; 
	int16u cluster_2 = 0xffff; int8u num2 = 0xff; int8u level = 0xff;
	int16u cluster_3 = 0xffff; int8u num3 = 0xff; 
	int16u color_temp = 0xffff; int16u current_X = 0xffff; int16u current_Y = 0xffff;
	...
	group_id = *((int16u*)&rBuf[2]);
	scene_id = rBuf[4];
	transition_time = *((int16u*)&rBuf[5]);
	// get on-off 
	cluster_1 = *((int16u*)&rBuf[7]);
	num1 = rBuf[9];
	on_off = rBuf[10];
	// get level-control
	cluster_2 = *((int16u*)&rBuf[11]);
	num2 = rBuf[13];
	level = rBuf[14];
	// get color-control
	cluster_3 = *((int16u*)&rBuf[15]);
	num3 = rBuf[17];
	color_temp = *((int16u*)&rBuf[18]);
	current_X = *((int16u*)&rBuf[20]);
	current_Y = *((int16u*)&rBuf[22]);
	
	zclBufferAddWord(group_id);
    zclBufferAddByte(scene_id);
	zclBufferAddWord(transition_time);
	get_group_name(scene_name,scene_id);
	zclAddString(scene_name, 18);

	zclBufferAddWord(cluster_1);
	zclBufferAddByte(num1);
	zclBufferAddByte(on_off);
	
	zclBufferAddWord(cluster_2);
	zclBufferAddByte(num2);
	zclBufferAddByte(level);
	
	zclBufferAddWord(cluster_3);
	zclBufferAddByte(num3);
	zclBufferAddWord(color_temp);
	zclBufferAddWord(current_X);
	zclBufferAddWord(current_Y);
	...
	if (EMBER_SUCCESS != status)
	{ 
		SengledSendZclResponse(CMD1_ZIGBEE_TYPE, CMD2_ADD_SCENES, status, node_id, rBuf);
	}
                                                     ...
                                                     int8u sceneId)
{
	...
	sBuf = GetSendBuffer();
	*((int16u*)sBuf) = CMD1_ZIGBEE_TYPE; sLen += 2;  //SUBSYS_ID
	*((int16u*)&sBuf[sLen]) = CMD2_ADD_SCENES; sLen += 2; //CMD_ID
...
void ProcessRecallScene(int8u *rBuf, int8u len)
{
	int16u group_id = 0xffff;
	int8u scene_id = 0xff;
	EmberStatus status = 0xff;
...
#ifdef OLD_VER	
	group_id = *((int16u*)&rBuf[0]);
	scene_id = rBuf[2];
	zclBufferAddWord(group_id);
    zclBufferAddByte(scene_id);
	
	EmberStatus status = SengledSend(group_id, TRUE);

#else
	int16u broadcast_addr = *((int16u*)&rBuf[0]);
	group_id = *((int16u*)&rBuf[2]);
	scene_id = rBuf[4];
	zclBufferAddWord(group_id);
	zclBufferAddByte(scene_id);

	if(broadcast_addr >= EMBER_BROADCAST_ADDRESS)
	{
		status = SengledSend(broadcast_addr, FALSE);
	}
	else
	{
		status = SengledSend(group_id, TRUE);
	}
	...
	if (EMBER_SUCCESS != status)
	{ 
		SengledSendZclResponse(CMD1_ZIGBEE_TYPE, ZCL_SCENES_CLUSTER_ID, status, group_id, rBuf);
	}
...
void (*process_wifi_to_zc_cmd_network[3])(int8u*, int8u) = {
	ProcessCreateNetworkReq,
	ProcessRemoveDeviceReq,
	ProcessFindNewNodeId
};
...
void DataParse(int8u *rBuf, int8u len)
{
	...
	// cmd alignment, to improve efficiency;
	switch (cmd1)
	{
...
{
	halGpioConfig(PORTC_PIN(1), GPIOCFG_IN_PUD);
#ifdef SUPPORT_JIAJU_PROJECT   // em3581 + Z02 RF board
	if (26 == channel)	// Z02 ?
	{ 
		return -43;
	}
...
void processDebugInfo(int16u debug_info)
{
	...
	*((int16u*)&sBuf[sLen]) = 0xff; //CMD_ID
	sLen += 2; 
	*((int16u*)&sBuf[sLen]) = debug_info; //flag
	sLen +=2;
