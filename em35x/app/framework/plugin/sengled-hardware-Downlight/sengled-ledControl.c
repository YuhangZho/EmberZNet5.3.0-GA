...
void powerReportFunction(void)
{
  ...
  else
  {
    //Computing Power 
#ifdef ROBIN_DEBUG
  emberAfGuaranteedPrintln("ADC-current: %2x,ADC-voltageIn: %2x",adcData[0],adcData[1]); // Robin add for debug
#endif
...
#ifdef ROBIN_DEBUG
  emberAfGuaranteedPrintln("currentIn: %4x,voltageIn: %4x",currentIn,voltageIn); // Robin add for debug
#endif
...
void SetLedLevel(int8u level)
{
  ...
  dimLevel = 1067*level/10+2787;
  
  if(dimLevel < 3000)  // 3000 means 10%
  {
	dimLevel = 3000;
  }
...
#ifdef ROBIN_DEBUG	
	emberAfGuaranteedPrintln("LevelPwm: %4x",dimLevel); // Robin add for debug
#endif	 
...
void SetLed(boolean onOffStatus, int8u currentLevel, int16u colorTpt)
{
  ...
  else if(onOffStatus == LED_OFF || currentLevel == 0x00)
  {
	...
	//emberEventControlSetDelayMS(offDelaySetPB6EventControl,2000);
	emberEventControlSetDelayMS(powerReportEventControl,1000);
  }
}

void softwarePowerUp(void)
{
    ...
    emberAfReadServerAttribute(emberAfPrimaryEndpoint(),
								  ...
								  (int8u *)&currentLevel,
								  sizeof(int8u)); 	
	if(currentLevel >= POWERUP_LEVEL_STABLE)
	{
		for (int16u i=POWERUP_LEVEL_STABLE; i<=currentLevel; i++)		
		{
			SetLedLevel(i);
...
#ifdef ROBIN_DEBUG
	emberAfGuaranteedPrintln("SOFTWAREPower"); // Robin add for debug
#endif	
}
                                                       ...
                                                       EmberAfAttributeId attributeId)
{
  ...
  emberAfReadServerAttribute(endpoint,
							...
							(int8u *)&currentColorTemperature,
							sizeof(int16u));
  if(onOffOnce != onOffStatus)
  {
	  onOffOnce = onOffStatus;
