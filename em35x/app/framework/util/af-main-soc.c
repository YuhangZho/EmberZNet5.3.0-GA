...
#ifdef HUBS02_3581
  // for Z03 Gateway(HUBS02_3581 / Element Z03Hub)
  // need boost mode
  // external PA, Config PC5, PC6 as TX RX
  ////////////////////////////////////////////////////////
...
volatile static int8u new_round_start = 0;
volatile static int32u tx_fail_cnt = 0;

EmberEventControl TestSendFailCntControl;
                                 ...
                                 EmberMessageBuffer message)
{
  ...
  /////////////// for 707 rf test  ////////////////
  // if receive success, means 707 works well, clear and disable the timer
  new_round_start = 0;
  tx_fail_cnt = 0;
  emberEventControlSetInactive(TestSendFailCntControl);
  emberAfGuaranteedPrintln("*********RECEIVE clear the calculator*********");
  ///////////////////////////////////////////
...
void TestSendFailCntFunction(void) 
{
    emberEventControlSetInactive(TestSendFailCntControl);  
	
	tx_fail_cnt++;
	emberAfGuaranteedPrintln("tx_fail_cnt : 0x%2X", tx_fail_cnt);
	if(tx_fail_cnt >= 10) // more than 1s, give a hint to tester, then tester should stop vivado by hand....
	{
	    while(1)
	    {
        	emberAfGuaranteedPrintln("###################STOP THE VIVADOL!!!!!!!!!!!!!\n\n\n\n");
	    }
	}
	
    emberEventControlSetDelayMS(TestSendFailCntControl, 100);  
}
                             ...
                             EmberStatus status)
{
  ...
  if((status == 0x66) && (new_round_start == 0)) // tx 66  send fail
  {
    // start a timer
    new_round_start = 1;
    //emberEventControlSetActive(TestSendFailCntControl);  
  }
  else if(status == 0x00)
  {
	...
	tx_fail_cnt = 0;
	emberEventControlSetInactive(TestSendFailCntControl);
	emberAfGuaranteedPrintln("*********SEND clear the calculator*********");
  }
