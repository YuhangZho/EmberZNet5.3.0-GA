...
//sengled 2015.9.29 wms
int8u saveEndpoint = 1;
//------------------------------------------------------------------------------
...
// ******************************************************************
extern boolean disable_permit_join_from_device;
int16u get_addr_list[64]={0};
int8u addr_list_ptr = 0;
//extern EmberEventControl ReadAttrTestControl;
                       ...
                       int16u length)
{
  ...
  if (apsFrame->clusterId == SIMPLE_DESCRIPTOR_RESPONSE) {
    ...
    emberAfAppPrintln("Device Announce: 0x%2x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,",
                      (int16u)(message[1])
                      + (int16u)(message[2] << 8),
                      message[3],
                      message[4],
	  ...
	  EmberNodeId target = (int16u)(message[1]) + (int16u)(message[2] << 8);
	  boolean removeChildren = 1;
	  boolean rejoin = 0;
	...
	get_addr_list[addr_list_ptr] = (int16u)(message[1]) + (int16u)(message[2] << 8);
//#ifdef SUPPORT_707TEST
	//SengledTestClusterCommand(get_addr_list[addr_list_ptr]); 
	addr_list_ptr++;
	if(addr_list_ptr >= 64) // 64 is max point could hold for em35x;
	{
		addr_list_ptr = 0;
	}
	// start trigger read attr continuous test
	// to capture 1-3s 707 no receive
	//emberEventControlSetDelayMS(ReadAttrTestControl, 5000);  
//#endif	
        
#ifdef SUPPORT_NODE_ID_CONFLICT	
	// enable device node id update, only when shut down the permit join 
	if(disable_permit_join_from_device == TRUE)
	{
		EmberNodeId nodeId = 0xffff;
		EmberEUI64 nodeEUI64;

		nodeId = (int16u)(message[1]) + (int16u)(message[2] << 8);
		MEMCOPY(nodeEUI64, message+3, EUI64_SIZE);

 		NewDeviceJoinReport(nodeId, nodeEUI64);
	}
#endif
